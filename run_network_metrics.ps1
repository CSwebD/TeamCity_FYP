...
# Prepare CSV header if it doesn't exist (using proper newline `r`n)
if (!(Test-Path $performanceCSV)) {
    [System.IO.File]::WriteAllText($performanceCSV, "Timestamp,DNS Lookup (ms),TCP Connection (ms),SSL Handshake (ms),TTFB (ms),Response Time (ms),DOM Content Loaded (ms),Total Load Time (ms)`r`n", [System.Text.Encoding]::UTF8)
}
